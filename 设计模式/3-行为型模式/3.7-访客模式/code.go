...
type IOrderService interface {
	...
	// Accept 有的教程里把接收 visitor 实现的方法名定义成 Accept
	Accept(visitor IOrderVisitor)
}

type IOrderVisitor interface {
	// Visit 这里参数不能定义成 IOrderService
	Visit(order *Order)
	Report()
}

// OrderService 销售订单服务，实现IOrderService接口
...
// CityVisitor 区域销售报表, 按城市汇总销售情况, 实现IOrderVisitor接口
